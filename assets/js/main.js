...
(function ($) {
	...
	if ($('.cta-text').length > 0) {
		...
		let chars = mySplitText.chars;
		let endGradient = chroma.scale(['#Ff0000', '#Ff0000', '#Ff0000', '#Ff0000', '#Ff0000']);
		cta.to(chars, {
			duration: 0.5,
			scaleY: 0.6,
			ease: "power1.out",
	...
	if ($('.tp-main-menu-content').length && $('.tp-main-menu-mobile').length) {
		...
		// Remove dropdown toggling behavior entirely
		$('.tp-main-menu-mobile .has-dropdown > a').off('click');
		$('.tp-main-menu-mobile .tp-submenu').removeAttr('style');
	}
